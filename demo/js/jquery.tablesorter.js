(function($) {
    $.extend({tablesorter:new
            function() {
                var parsers = [],widgets = [];
                this.defaults = {cssHeader:"header",cssAsc:"headerSortUp",cssDesc:"headerSortDown",cssChildRow:"expand-child",sortInitialOrder:"asc",sortMultiSortKey:"shiftKey",sortForce:null,textExtraction:"simple",parsers:{},widgets:[],widgetZebra:{css:["even","odd"]},headers:{},widthFixed:false,cancelSelection:true,sortList:[],headerList:[],dateFormat:"us",decimal:'/\.|\,/g',onRenderHeader:null,selectorHeaders:'thead th',debug:false};
                ...
                function buildHeaders(table) {
                    ...
                    // code below added to add the duplicated table header THs to the returned  $tableHeaders to allow for clicking on them to sort
                    $tableHeaders2 = $(table.config.selectorHeaders, $(".fht-thead .fht-table")[0]).each(function(index) {
                        this.column = header_index[this.parentNode.rowIndex + "-" + this.cellIndex];
                        this.order = formatSortingOrder(table.config.sortInitialOrder);
                        this.count = this.order;
                        if (checkHeaderMetadata(this) || checkHeaderOptions(table, index))this.sortDisabled = true;
                    ...
                    // merge $tableHeaders2 INTO $tableHeaders
                    $.extend($tableHeaders, $tableHeaders2);
    ...
    ts.addParser({id:"currency",is:function(s) {
        ...
        return $.tablesorter.formatFloat(s.replace(new RegExp(/[ÂL$â‚¬]/g), ""));
    },type:"numeric"});
